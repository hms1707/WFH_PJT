...
with head_left:
    ...
    <div style="display: flex; align-items: flex-end; gap: 20px; padding-bottom: 5px;">
        ...
        <div style="display: flex; flex-direction: column; font-size: 0.85rem; color: #aaa; line-height: 1.3;">
            <div style="display: flex;"><span style="width: 75px;">Engine</span><span>v3.0 (2026.07.02)</span></div>
            <div style="display: flex;"><span style="width: 75px;">Dashboard</span><span>v3.0 (2026.07.02)</span></div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
with mid:
    ...
    def render_chart():
        ...
        try:
            ...
            # ★ 버그 픽스: v0.3 방식의 현재 가격 가로 대시선 및 Y축 정밀 라벨 박스 그리기
...
with right:
    ...
    def render_history():
        ...
        if not hist.empty and all(c in hist.columns for c in needed_cols):
            ...
            st.caption("클라우드 DB에 체결 내역이 없거나 데이터를 불러오는 중입니다...")
